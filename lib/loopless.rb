...
module GoingLoopless
  ...
  def find_all_in_role(role_name, people)
    results = people.select do |person|
      person.roles.any? { |role| role.name == role_name }
    end
  end
  ...
  def list_movies(person)
    movies = person.roles.map(&:movie).uniq.sort_by(&:year)
  ...
  def build_credits(movie, role_order)
    results = role_order.flat_map do |role_name|
