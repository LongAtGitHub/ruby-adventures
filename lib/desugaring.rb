module DesugaringExercises
  ...
  def desugared_interpolation(recipients, event, message)
    mail(message,
    {:to => recipients.map {|x| x.email }, :subject => "You’re invited to " + event.title.to_s + " on " + event.date.to_s})
  end
  ...
  def desugared_operators(recipients, event, message)
    self.mail(message,
    {:to => recipients.map {|x| x.email() }, :subject => (
      "You’re invited to ".+(
        event.title.to_s().+(" on ".+ (event.date.to_s())))
    )})
  end
